(function () {
  ...
  async function renderList() {
    try { STATE.envelopes = await api('/api/envelopes'); } catch (e) { return `<p class="banner banner-warn">Could not load envelopes: ${escapeHtml(e.message)}</p>`; }
  ...
  async function sendEnvelope() {
    ...
    try {
      const result = await api('/api/envelopes', { method: 'POST', body: fd });
  ...
  async function renderDetail() {
    let env;
    try { env = await api('/api/envelopes/' + STATE.currentId); } catch (e) { return `<p class="banner banner-warn">${escapeHtml(e.message)}</p>`; }
    ...
    const rows = env.signers.map((s, i) => {
      ...
      if (s.status === 'signed') status = `<span class="sig-status signed">Signed ${formatDateTime(s.signed_at)}</span>`;
        ...
        link = s.sign_token ? `<button class="btn btn-sm btn-ghost" data-copy="${location.origin}/sign/${s.sign_token}">Copy signing link</button>` : '';
      } else status = `<span class="sig-status waiting">Waiting for turn</span>`;
      const sigImg = s.has_signature ? `<img class="sig-img" src="/api/envelopes/${env.id}/signers/${s.id}/signature">` : '';
    ...
    const completedBlock = env.status === 'completed' ? `
      ...
      <div style="margin-top:14px;"><a class="btn btn-primary" href="/api/envelopes/${env.id}/download">Download executed document</a></div>` : '';
  ...
  async function loadDetailPreview(env) {
    ...
    try {
      if (env.source_type === 'pdf') {
        const buf = await (await fetch(`/api/envelopes/${env.id}/file`, { credentials: 'same-origin' })).arrayBuffer();
        ...
        let html = '';
        for (let i = 0; i < env.page_count; i++) html += `<img class="doc-page-img" src="/api/envelopes/${env.id}/pages/${i}">`;
        el.innerHTML = html;
      }
